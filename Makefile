...
all: client_bin server_bin test_crypto_bin mitm_bin
...
mitm_bin: mitm/mitm.c mitm/dh.c crypto/crypto.c
	gcc $(CFLAGS) mitm/mitm.c mitm/dh.c crypto/crypto.c \
	-o mitm/mitm $(LDFLAGS) $(LIBS)
...
clean:
	rm -f client/client server/server crypto/test_crypto mitm/mitm